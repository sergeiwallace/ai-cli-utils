...
class TestGetClaudeUsage:
    def test_usage_returns_none_pending_hidden_pane_implementation(self):
        """_get_claude_usage_percent returns None until AI-CLI-22 lands.

        Active-pane scraping was disabled to stop /usage being injected into
        live CC sessions. The hidden-pane implementation will replace this.
        """
        with patch("ai_cli.quota._scrape_usage_tmux", return_value=None):
