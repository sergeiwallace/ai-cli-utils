...
@pytest.mark.parametrize(
    ("delta", "expected_delta", "expected_color"),
    [
        (-5.0, "-5%", "\x1b[38;2;48;160;178m"),
        (3.0, "+3%", "\x1b[38;2;243;141;16m"),
        (15.0, "+15%", "\x1b[38;2;220;38;38m"),
    ],
)
def test_given_pace_case_when_rendered_then_every_provider_uses_shared_signed_gradient(
    adapter, label, delta, expected_delta, expected_color
):
    ...
    assert ANSI_ESCAPE.sub("", rendered) == f"{label} {usage_pct:.0f}% ↑ {expected_delta}"
