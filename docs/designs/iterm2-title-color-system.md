...
| Bug 7 — Cross-session pane name clobbering | When session B is spawned from session A's shell, it inherits A's `ITERM_SESSION_ID`. Fixed with three changes: (1) `_iterm2_fleet_setup` / `_iterm2_status` guard the rename behind `tmux list-clients` — detached sessions skip it; (2) the re-attach path in `_do_session_launch` writes the current pane's GUID into the session's tmux environment so the stale inherited GUID is overwritten on next attach; (3) the first-run `set-iterm2-name` block using the static `$ITERM_SESSION_ID` shell variable was removed — it used the original inherited GUID (never updated in the shell process), which reactivated after `ai update` triggered `exec bash new-template` resetting `first_run=true`. `_iterm2_fleet_setup` already handles the rename via `_live_iterm_id()`, making the static-var block redundant and harmful. |
