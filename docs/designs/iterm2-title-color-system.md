...
| Bug 7 — Cross-session pane name clobbering | When session B is spawned from session A's shell, it inherits A's `ITERM_SESSION_ID`. Fixed with four changes: (1) `_iterm2_fleet_setup` / `_iterm2_status` guard the rename behind `tmux list-clients` — detached sessions skip it; (2) the re-attach path in `_do_session_launch` writes the current pane's GUID into the session's tmux environment so the stale inherited GUID is overwritten on next attach; (3) the first-run `set-iterm2-name` block using the static `$ITERM_SESSION_ID` shell variable was removed (reactivated after `ai update` reset `first_run=true` via template refresh); (4) `_emit_iterm2_profile_setup` now reads the live GUID from `tmux show-environment` when called inside tmux — the shell-env `$ITERM_SESSION_ID` is the original creation GUID (never updated in the running process), so renaming with it would target the original creation pane instead of the current pane. |
