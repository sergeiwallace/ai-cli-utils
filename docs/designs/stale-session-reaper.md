...
- **Deliverables:**
  ...
  - Files modified: `src/ai_cli/main.py`, `src/ai_cli/session_script.py`, `src/ai_cli/process_probe.py`, `src/ai_cli/config.py`, `tests/test_runaway_loop_guards.py`, `tests/test_cli.py`, `tests/test_session_self_update.py`, and `tests/test_config_watch_hash.py`.
  ...
  - **T-1.5 Integrated safety regression coverage**
    ...
    - [ ] When real supervisor/child subprocesses exercise local and remote fast exits, normal child exits, child crashes, and supervisor crashes, the system shall prove local EXIT cleanup, remote interactive-shell recovery with retained lease/record until shell exit, ordinary restart behavior, immediate crash release, and zero unauthorised kill attempts on every supported platform.
    - [ ] When generated-script regressions cover watcher pacing, direct self-`exec` replacement, self-update, and config-change monitoring, `tests/test_runaway_loop_guards.py`, `tests/test_cli.py`, `tests/test_session_self_update.py`, and `tests/test_config_watch_hash.py` shall be updated for the supervisor/child structure and shall continue passing rather than being deleted.
