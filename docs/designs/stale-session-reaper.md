...
At session creation, tmux starts one stable supervisor as the pane leader. It creates the generation token, writes it once to tmux metadata, acquires and verifies the exclusive generation lease through the package's Python `portalocker` backend, and only then starts its heartbeat-publishing loop and writes the first usable record. The shell keeps the locked descriptor open for its own process lifetime; the short Python helper locks that inherited descriptor and exits, so it does not become a lease holder. The heartbeat publisher is a detached, terminal-free companion process which closes any inherited lease descriptor before it starts its approximately 30-second `WORKING` ticks. It continues across agent child generations and is never restarted per attempt. `start_watcher()` is now explicitly limited to replacing its prior watcher, clearing its lock file, and checking Gemini reload/restart marker files once per second. It does not hash configuration, detect idle prompts, read exit-signal markers, or inject input into a pane. `tests/test_runaway_loop_guards.py` covers the real one-second pacing, while `tests/test_main.py` asserts that generated scripts contain none of the retired exit-injection markers or commands. On final supervisor shutdown, the supervisor revokes only its exact-generation record and releases the lease. If the supervisor crashes, the OS releases its lease immediately; a retained record is not evidence of a live session and can authorize reap only after the ordinary stale-heartbeat, process-identity, and lease-held revalidation gates. No second process can survive in a different lease state. If the Python lock backend cannot acquire the descriptor, the supervisor logs `generation lease backend unavailable`, publishes no usable evidence, and preserves the session.

The existing full wrapper body becomes an executable child body. The supervisor detects a changed stable-script mtime before each spawn and the existing version/commit mismatch after a child exits. Instead of any of the three direct self-`exec` sites, it starts the selected fresh script in an internal child-body mode and waits for it to exit, then rechecks update conditions before the next spawn. When it has a terminal, the supervisor preserves the original terminal descriptor, starts the child through a short Python `exec` wrapper, and waits for the child to create its own process group before transferring the terminal foreground group to it. The wrapper restores stdin from the preserved descriptor and resets `SIGINT` and `SIGQUIT` before the child shell starts; it stops itself until the supervisor has completed the transfer. This gives the child genuine foreground terminal input and terminal-generated signals while the supervisor remains blocked in its interruptible `wait` builtin. The child retains the current body behavior: `direnv exec` agent launch, handoff-pending pickup, status/event publication, `first_run` behavior, and the existing fast-exit stop condition. For a local session, an agent exit in under three seconds makes the supervisor perform its normal EXIT cleanup—revoking its exact-generation record, releasing its lease, and exiting—just as the current outer loop ends the session. For a remote session, that same fast exit retains the current recovery behavior: the child body emits the existing stop diagnostic and runs an interactive shell; when that shell completes, the child returns a distinct recovery-complete status and the supervisor performs final cleanup. Heartbeat publication continues unchanged during remote recovery, including `WORKING` ticks, exactly as it does during ordinary agent execution; only the per-child monitoring state stops because the interactive recovery shell is not an agent to monitor. A later ordinary agent exit returns status 0 for both local and remote sessions, so the supervisor starts the next agent child while retaining its lease. A child crash or exit never releases the supervisor's lease and therefore cannot create a free-lease/live-supervisor state.
...
For a terminal-backed child, the Python exec wrapper creates a separate child process group and stops before exec. While the supervisor is still the terminal foreground group, it promotes that child group with `tcsetpgrp()` and resumes it. The child therefore receives normal terminal stdin, Ctrl-C, and terminal resize delivery directly from the terminal driver. The supervisor then remains in its background process group, blocked in an interruptible `wait`; a signal addressed directly to its PID still runs its trap promptly. The detached ticker has no controlling terminal and no signal or stdin role. **Revision 3 D-10 signal refinement:** POSIX shell traps provide no signal-origin metadata, so the supervisor's `SIGINT` and `SIGWINCH` traps have one uniform, observable behavior for direct delivery: record-only and never relay. Only a `SIGTERM` addressed to the supervisor is relayed, exactly once, to a live direct child PID; because the supervisor is waiting in the interruptible builtin, its trap runs promptly even while the child is live. The child-body signal trap relays that copy to its currently active agent process before exiting or resuming its normal signal behavior. If the child exits during that relay, the supervisor performs no second relay. The supervisor waits for child termination before final cleanup. The supervisor alone owns the existing side-effecting EXIT cleanup (watcher and signal-watch shutdown; lock, handoff, metadata, and configuration-file removal; worktree and color-slot release; session-file cleanup) plus exact-generation record revocation. The child performs none of that final cleanup, preventing duplicate release or deletion after each ordinary restart. The generated template currently prefers zsh and falls back to Bash, so real generated-shell subprocess tests on supported macOS, Linux, and WSL environments must verify foreground input, direct Ctrl-C and resize propagation, record-only supervisor-directed `SIGINT`/`SIGWINCH`, exactly-once supervisor-directed `SIGTERM` forwarding, and ticker continuity. If that verification cannot establish those properties, the implementation must preserve the session and disable reap evidence on that platform rather than silently use an unverified signal model. Native Windows without the tmux/shell runtime is outside this wrapper path.
...
`session_script.py` is structurally split into a persistent pane-leader supervisor, a child-body entry point promoted into its own pane foreground process group, and a detached terminal-free heartbeat ticker. The supervisor creates the generation token, writes `@ai_cli_session_generation`, holds the lease descriptor for the generation, and replaces all three direct self-`exec` sites with child exec/wait cycles. Before the child shell starts, a short Python exec wrapper restores the preserved terminal descriptor, creates and stops in its own process group, resets the background-job `SIGINT`/`SIGQUIT` dispositions, and waits for the supervisor's `tcsetpgrp()` promotion. This lets the supervisor wait interruptibly without sacrificing terminal signal or input delivery to the child. Its internal Python/`portalocker` helper locks the inherited descriptor so the supervisor remains the sole lifetime owner on macOS and Linux without depending on a `flock(1)` executable. The child body retains the agent/restart behavior while the supervisor survives stable-script and self-update cycles; remote recovery completion uses a distinct status while ordinary remote exits restart. The internal heartbeat handler adds supervisor-authorised, generation-conditional ledger persistence before its best-effort message publication; a local write failure does not suppress that publication attempt. `process_probe.py` adds the public `ProcessIdentity` type and `capture_identity()` contract, implemented by both existing probe backends. Existing event publication remains unchanged. `process_manager.py` gains Circus registration/removal helpers beside existing watchers, using the existing XDG state directory and package-resolved executable. `main.py` dispatches the management and internal run commands. The metadata option, rather than any session-name pattern, is the sole authoritative managed-session marker; this includes valid indexed, custom, hyphenated, local, remote, and `cx` session names.
...
- **Deliverables:**
  ...
  - **T-1.4 Supervisor/child lifecycle and terminal control**
    - [ ] When a user presses Ctrl-C or the terminal resizes while the separately grouped child is the pane foreground process group, the system shall deliver the signal directly to the child exactly once, shall not relay it from the supervisor, and shall preserve interactive terminal input through the supervisor's saved terminal descriptor.
    ...
    - [ ] When a child exits or crashes after a non-fast agent run, the system shall recheck update conditions and spawn the next agent child for both local and remote sessions while retaining the supervisor PID and continuous lease; when an agent exits in under three seconds, a local supervisor shall perform normal EXIT cleanup and terminate the session, while a remote supervisor shall wait on an interactive-shell child without revoking its record or releasing its lease until that shell exits, then receive the distinct recovery-complete status and perform final cleanup, with its detached heartbeat ticker continuing to tick while that shell is live.
    ...
    - [ ] If the foreground child terminal model, detached ticker, signal relay, or Python lock backend cannot be established and verified on a supported tmux/zsh-or-Bash platform, then the system shall create no usable heartbeat evidence, log a preserve reason, and preserve the session.
  - **T-1.5 Integrated safety regression coverage**
    ...
    - [ ] When real generated-shell subprocesses under both zsh and Bash exercise terminal Ctrl-C and resize, supervisor-directed `SIGINT`, `SIGWINCH`, and `SIGTERM`, and child exit during `SIGTERM` relay, the system shall prove exact one-delivery counts, no `SIGINT`/`SIGWINCH` relay, exactly-once `SIGTERM` relay, preserved interactive input, detached ticker continuity, and zero unauthorised kill attempts on every supported platform.
    - [ ] When real supervisor/child subprocesses exercise local and remote fast exits, normal child exits, child crashes, and supervisor crashes, the system shall prove local EXIT cleanup; remote interactive-shell recovery with a retained lease and record plus at least one further `WORKING` heartbeat while the shell is live; distinct remote recovery completion; ordinary restart behavior for remote status-0 exits; immediate crash release; and zero unauthorised kill attempts on every supported platform.
    - [ ] When generated-script regressions cover watcher pacing, direct self-`exec` replacement, and self-update, `tests/test_runaway_loop_guards.py`, `tests/test_cli.py`, `tests/test_main.py`, and `tests/test_session_self_update.py` shall continue passing. `tests/test_runaway_loop_guards.py` shall exercise the current one-second watcher pacing, and `tests/test_main.py` shall prove generated scripts do not retain the retired exit-injection markers or commands.
    - [ ] **Parity reconciliation (2026-09-01, commit `10bae49`):** The signal-file idle-prompt detector, configuration-hash/configuration-change state, and `tests/test_config_watch_hash.py` were deliberately removed with the `/exit` auto-injection watcher. This AC retires their preservation rather than restoring behavior whose consumer was intentionally deleted; the current explicit no-auto-injection contract is covered by the tests named above.
