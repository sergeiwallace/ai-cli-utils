...
> [AIH-334](../../../../../ai-harness/docs/roadmap/master-roadmap.md) (ai-harness roadmap) — local
...
> internals write-ups). **D-3 stays open until AIH-334 reports back.** If a safe in-place reconnect is
> confirmed, D-3 is revised to prefer it (and D-3(b)'s deferred confirmed-restart-command may also
> unblock, since AIH-334's finding would resolve the "unverified premise" objection that deferred it).
> If AIH-334 confirms no lever exists, (a) ships as originally recommended with that negative result
