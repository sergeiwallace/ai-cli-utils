...
**Research basis:** `~/projects/aido/docs/research/gemini-research-pipeline-synthesis.md` (aido R-19 — migrated from sergei R-61+R-62)
...
Informed by aido R-22 / sergei R-66 (2026 gap analysis, 2026-04-06). Not blocking Phase 1 implementation — ship `--depth standard` and `--depth deep` first, then evaluate these enhancements.
...
### P2-4: Gemini Deep Research via Interactions API (`--depth exhaustive`)

As of early 2026, Gemini's actual Deep Research capability (the post-trained agentic harness, not just Gemini + grounding) is accessible via the official async `Interactions API` (`ai.google.dev/gemini-api/docs`). This is a different product than calling `gemini-2.0-flash-thinking` with `googleSearch` tools — it's the same pipeline that powers the consumer Deep Research UI.

Capabilities: multimodal inputs, GCS bucket reads up to 100MB, private web data (Drive, Gmail transcripts), async polling (long-horizon tasks, minutes not seconds).

**Proposed `--depth exhaustive` tier:**
- Delegates to the Gemini Deep Research harness via Interactions API rather than orchestrating ourselves
- Async-first: submit task → poll for completion → retrieve output (aligns with our `--resume` checkpoint pattern)
- Scope: research tasks requiring true recursive browsing + multi-source synthesis beyond what `--depth deep` can produce

*Impact:* Unlocks production-grade deep research via a single API call. Trade-offs: higher cost, async-only (no streaming), GCP dependency. Evaluate cost/quality vs `--depth deep` on DRACO benchmark tasks before committing.

*Depends on:* researching current Interactions API pricing and quota model (add to SW-766 scope).
...
| 2026-04-06 | Round 5 | Phase 2 enhancements added based on aido R-22 / sergei R-66 (2026 gap analysis): P2-1 IEW voting for --depth deep, P2-2 hierarchical search provider, P2-3 token-level fault tolerance (confirmed no action — existing checkpointing is correct ceiling). |
