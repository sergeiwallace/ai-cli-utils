...
| 2026-04-19 | Patch-target unification (AI-CLI-47) | ``main.py`` imports extracted modules as underscore-prefixed namespaces (``_config``, ``_handoff``, ...) and uses qualified call sites (``_config.load_config()``). Test patches for names that live in extracted modules now target the owning module — ``patch("ai_cli.config.load_config")`` — via a scripted sweep of 407 substitutions across 9 test files. Re-exports retained for backwards compatibility. |
| 2026-04-19 | Pre-existing test fixes (AI-CLI-47) | Fixed two deploy-test failures (uv binary path resolution) and four quota-scrape test failures (new-session/kill-session after 3caa545). Both sets were pre-existing on ``main`` and unrelated to the Click migration; addressed here so the Phase 2 green gate is meaningful. |
