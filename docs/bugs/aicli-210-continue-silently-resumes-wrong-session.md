...
| 2026-08-12 | Pending (`AI-CLI-cc-session-live-mvht`) | The guard above matched on `sessionId` alone and never checked the pid, so an abandoned record refused its session name forever (`ai c session-1`: "still running (pid 3076127)" against a pid with no `/proc` entry; four such records cleared by hand on one box). A record now counts as live only when the pid runs **and** field 22 (`starttime`) of `/proc/<pid>/stat` matches the record's `procStart`, so a recycled pid cannot reintroduce the false refusal; records for provably-gone pids are pruned while the directory is walked. Unverifiable records fail **open** (launch allowed, record kept), which is the deliberate opposite of the worktree liveness probe that guards a deletion. |
