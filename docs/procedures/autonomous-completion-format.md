...
**BMS repo work — extended flow:** "Autonomous work completes" means the bot-findings + squash loop in `claude-base.md` has fully resolved. The UAT Summary is presented only after: (a) all Wiz + CC review bot findings are addressed (including low-priority / informational), AND (b) commits are squashed into the agreed atomic set with a clean post-squash bot-findings pass.
